  ...
  el.innerHTML = `
    <div class="settings-section">
      ...
      <div class="setting-row">
        <div class="setting-info"><div class="setting-name">Mask credentials</div><div class="setting-desc">Blur passwords by default in detail view</div></div>
        <button class="toggle${s.mask ? " on" : ""}" data-setting="mask"></button>
      </div>
      <div class="setting-row">
        <div class="setting-info"><div class="setting-name">Auto-lock</div><div class="setting-desc">Re-lock after 5 minutes of inactivity</div></div>
        <button class="toggle${s.autoLock ? " on" : ""}" data-setting="autoLock"></button>
      </div>
      <div class="setting-row">
        <div class="setting-info"><div class="setting-name">Clipboard clear</div><div class="setting-desc">Wipe clipboard 30s after copy</div></div>
    ...
    <div class="settings-section">
      ...
      <div class="setting-row">
        <div class="setting-info"><div class="setting-name">Particle field</div><div class="setting-desc">Animated canvas background</div></div>
        <button class="toggle${s.particles ? " on" : ""}" data-setting="particles"></button>
      </div>
      <div class="setting-row">
        <div class="setting-info"><div class="setting-name">Scanlines</div><div class="setting-desc">CRT overlay effect</div></div>
    ...
    <div class="settings-section">
      ...
      <div class="setting-row clickable" id="exportBtn">
        <div class="setting-info"><div class="setting-name">Export vault</div><div class="setting-desc">Download all credentials as JSON</div></div>
        <span class="setting-arrow">→</span>
      </div>
      <div class="setting-row clickable" id="clearBtn">
        <div class="setting-info"><div class="setting-name danger-text">Clear all data</div><div class="setting-desc">Permanently delete all credentials</div></div>
        <span class="setting-arrow danger-text">✕</span>
      </div>
    </div>
    <div class="settings-footer">Vault OS v2.0.0 · Local-only · No cloud sync</div>
  `;

  el.querySelectorAll(".toggle[data-setting]").forEach(btn => {
    btn.addEventListener("click", () => {
      const key  = btn.dataset.setting;
      const next = { ...getState("settings"), [key]: !getState("settings")[key] };
